...
def gen_data(n): 
    ...
    C = pd.read_csv(os.getcwd()+'/gain_matrix.csv',
                          encoding = 'utf-8',sep=',')
    C = array(C)
    ...
    ## input PMU variance
    sigma = pd.read_csv(os.getcwd()+'/PMU_variance.csv',
                         encoding = 'utf-8',sep=',')
    sigma = array(sigma)
...
def upd_inv_add(X, Xs, opti): 
    ...
    temp = np.multiply(sqrt(gamma[opti]), Xs[:,opti])
    Ys = Xs - (temp*temp.T)/(1+gamma[opti]*Xs[opti,opti])    
    return Y, Ys
...
def findopt(X, Xs, i, indexN, n,val):
    ...
    val = val + log(1-gamma[i]*Xs[i,i]) + log(1+gamma[opti]*Ys[opti,opti])
       
    return Y, Ys, opti, val

#### Compute the supgradient of objective function of M-SPATA ####
def find_k(x,s,d):
    for i in range(s-1):
        k = i
...
def grad_fw(fmat, x, s):
    ...
    fval = 0.0 # the current objective value
    for i in range(s):
        if i <= k:
